...
namespace Sonar {
	MainMenuState::MainMenuState(GameDataSptr data) : _data(data) {
	...
	void MainMenuState::Init() {
		...
		_data->assets.LoadTexture("Play Button", GAME_BUTTON_FILEPATH);
		...
		_title.setPosition( (SCREEN_WIDTH / 2) - (_title.getGlobalBounds().width / 2) , 
			_title.getLocalBounds().height / 2 );
	...
	void MainMenuState::HandleInput() {
		...
		while (_data->window.pollEvent(event)) {
			...
			if (_data->input.IsSpriteClicked( _playButton, sf::Mouse::Left, _data->window)) {
				_data->machine.AddState(StateUptr(new GameState(_data)), true);
			}
		}
	}
